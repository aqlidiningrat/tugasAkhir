...
def contourImage(idDB, img):
    ...
    print('*idDB:',idDB, ketImg)
    print('*fiturContour:')
    print({'aspectRatio':[aspectRatio], 'extent':[extent], 'solidity':[solidity],
...
def persentaseAkurasi(Xtrain, Xtest, y_train, y_test):
    ...
    print('=== autentikasi_tandaTangan', 'asli:',paMax, 'palsu:',paMin)
    return y_test, paMax, paMin

def klasifikasiKNN_fitur(fiturImg):
    df = pd.read_csv(os.path.join(root, 'fiturImg.csv'))
    Xtrain, y_train = df.drop(['filename','label'], axis=1), df['label']
    Xtest = pd.DataFrame(np.array([fiturImg]), columns=Xtrain.columns)
    X_train, X_test = dataPreprocessing(Xtrain, Xtest)
    model = knn.KNeighborsClassifier(n_neighbors=2, metric='euclidean')
    model.fit(X_train, y_train)
    y_predict = model.predict(X_test)
    return persentaseAkurasi(Xtrain, Xtest, y_train, y_predict)
...
def preprocessingCitra(namaTable, idDB):
    ...
    y_predict, paMax, paMin = klasifikasiKNN_fitur(fiturImg)
...
def autentikasiTandaTangan(namaTable, idDB, ketImg, y_predict, paMax, paMin):
    if (ketImg == 'True'):
        ...
        y_predict = 'terdeteksiNoise padaKotakPersegi padaGambar tandaTangan..'
