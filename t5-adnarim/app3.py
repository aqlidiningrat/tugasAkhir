...
app.config['UPLOAD_FOLDER'] = os.path.join(path, 'static', 'images')
...
def readTB_mirandaDB(namaTable, idDB):
    ...
    db.close()
    np_buffer = np.frombuffer(retrivied_bytes, np.uint8) # create nparray from buffer bytes
...
def klasifikasiKNN2(face_lb, lv):
...
def gen_frames():
    ...
    while True:
        if not camera.isOpened():
            print(f"Error: could not open camera with index {camera_index}, Triying next index")
            camera_index += 1
            continue
        else:
    ...
    while True:
        ...
        if not success:
            ...
            faces = faceDetector.detectMultiScale(frame_gray, scaleFactor=1.1, minNeighbors=5)

            # frame = low_brightnes(frame, lv)
            for (x,y,w,h) in faces:
                m = m+1
                img_face = frame[y:y+h, x:x+w]
                # img_low_brightnes = low_brightnes(img_face, lv)
                h_predict, paMax = klasifikasiKNN2(img_face, lv)
                colorRectandText = (242,51,70) if h_predict=='Laki-Laki' else (132,22,254)
                cv.rectangle(frame, (x+5,y), (x+w-5, y+h), colorRectandText, 1)
                cv.putText(frame, h_predict+' '+str(paMax)+'%', (x+5,y+10), cv.FONT_HERSHEY_DUPLEX, 0.3, colorRectandText, 1)
                cv.putText(frame, 'TA_miranda_2111017   wajah'+str(m), (x+5,y+h-5), cv.FONT_HERSHEY_DUPLEX, 0.2, (255,243,220), 1)
                # endfor

            print(len(faces),'wajahTerdeteksi')
...
def capture(idWebcam):
    ...
    insertTB_mirandaDB_withIndex('ambilGambarWebcam', idWebcam, 'capture_image.jpg', image_data)
...
def potongGambar(dari, idDB):
    ...
    return render_template('index2.html', render='potongGambar', dari=dari, idDB=idDB, idDBNew=idDBNew,
