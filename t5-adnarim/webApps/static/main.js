...
function cropImage(cavasEle, dari, idDB, idDBNew, url, hight=1, width=1) {
  ...
  let image = new Image();
  let scrHight = window.innerHeight - 154;
  ...
  const imageSave = function (blob){
      ...
      showAlert("aturKecerahanGambar..."); // alert
      const formData = new FormData(); // save image in root methods=['POST']
      formData.append('croppedImage', blob, 'cropped_image.jpg');
      fetch('/potongGambar/'+idDBNew, {
        method: 'POST',
        body: formData
      })
  ...
  setTimeout(function(){
    alert.innerText="klasifikasiCitra...";
  }, 1500);
  setTimeout(function(){
     // alert.style.display="none";
     alert.innerText=". . .";
     location.replace('/'+dari+'/'+idDB+'/potongGambar/'+idDBNew+'/aturKecerahanGambar_klasifikasiCitra');
   },2500);
  };
};
